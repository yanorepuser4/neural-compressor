...
class WeightOnlyLinear(torch.nn.Module):
    ...
    def pack_tensor(self, raw_tensor):
        ...
        for j in range(target_len):
            ...
            for e in range(tmp.shape[1]):
                tmp[:, e] &= mask
                # torch.bitwise_and(tmp[:, e], 2**self.bits - 1, out=tmp[:, e])
    ...
    def unpack_tensor(self, packed_tensor):
        wf = torch.tensor(
            list(range(0, self.compress_bits, self.bits)), dtype=torch.int32, device=self.device
        ).unsqueeze(0)
        unpacked_tensor = torch.bitwise_right_shift(torch.unsqueeze(packed_tensor, 2), wf.unsqueeze(0)).to(
            torch.uint8 if hasattr(self, "qzeros") else torch.int8
        )
        torch.bitwise_and(unpacked_tensor, 2**self.bits - 1, out=unpacked_tensor)
