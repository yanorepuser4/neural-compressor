...
def load(model_name_or_path, format="default", *model_args, **kwargs):
    if format == "huggingface":
        ...
        qmodel_file_path = os.path.join(os.path.abspath(os.path.expanduser(model_name_or_path)), WEIGHT_NAME)
        assert os.path.exists(qmodel_file_path), "Cannot load model from path {}".format(qmodel_file_path)
...
def _replace_with_woq_linear(model, quantization_config, loaded_state_dict_keys):
    ...
    for name, module in model.named_modules():
        if isinstance(module, torch.nn.Linear):
            if name + ".qweight" not in loaded_state_dict_keys:
                continue
            zp = quantization_config.get("zero_point", not quantization_config.get("sym", False))
            new_module = WeightOnlyLinear(
                module.in_features,
                module.out_features,
                bits=quantization_config.get("bits", 4),
                group_size=quantization_config.get("group_size", 32),
                dtype="int",
                zp=zp,
...
def _load_hf_woq_model(pretrained_model_name_or_path, *model_args, **kwargs):
    ...
    model = _replace_with_woq_linear(model, quantization_config, loaded_state_dict_keys)
