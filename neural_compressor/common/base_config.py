...
class BaseConfig(ABC):
    ...
    def _post_init(self, specific_params_list: Optional[list] = None):
        if self.white_list == DEFAULT_WHITE_LIST:
            global_config = self.get_params_dict(specific_params_list)
    ...
    def get_params_dict(self, specific_params_list: Optional[list] = None):
        result = dict()
        # if specific_params_list is given, then use it to generate params dict
        # else use default self.params_list
        params_list = self.params_list if specific_params_list is None else specific_params_list
