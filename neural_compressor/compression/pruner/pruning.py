...
class TickTockPruning:
    ...
    def __init__(self, config, model, total_pruned_cnt=40, completed_pruned_cnt=1):
        """Initialize."""
        self._model = model
        self.pruners = []
        self.pruners_info = process_config(config)
    ...
    def _generate_pruners(self):
        ...
        for info in self.pruners_info:
            ...
            if modules == {}:
                logger.warning("one pruner hooks no layers, please have a check")
            from .pruners import TickTockPruner
            self.pruners.append(TickTockPruner(info, modules,self.total_pruned_cnt, self.completed_pruned_cnt))
            info['modules'] = [key for key in modules.keys()]
            info['len_of_modules'] = len(info['modules'])
            logger.info(info)
