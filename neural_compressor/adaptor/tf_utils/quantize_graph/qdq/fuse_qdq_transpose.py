...
class FuseNodeStartWithTranspose(QuantizeNodeBase):
    ...
    def _add_transpose_function(self, original_node, quantized_op_node, dequantize_node):
        ...
        quantized_op_node.attr["T"].CopyFrom(attr_value_pb2.AttrValue(type=dequantize_node.attr["T"].type))
    ...
    def _apply_transpose_quantization(self, match_node_name):
        ...
        """
        # if match_node_name[1]=="StatefulPartitionedCall/vit/encoder/layer_._0/attention/attention/transpose":
        #     breakpoint()
        ...
        all_input_names = [q_inputs[0], normal_inputs[1]] + q_inputs[1:]
        ...
        for _, node in enumerate(self.input_graph.node):
            if node.name in skip_node_name:
                ...
                self._add_transpose_function(node, quantized_transpose_node, dequantize_node)
