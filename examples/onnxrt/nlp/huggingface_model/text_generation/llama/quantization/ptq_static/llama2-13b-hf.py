...
class KVDataloader:
    def __init__(self, model_path, pad_max=196, batch_size=1, sub_folder='train'):
        ...
        #dataset = datasets.load_from_disk('/mnt/data1/mengni/pile')['train']
...
if __name__ == "__main__":
    ...
    if args.tune:
        ...
        if args.weight_only and args.algorithm != "None":
            for model in ['decoder_model.onnx']:
                shutil.rmtree(args.output_model, ignore_errors=True)
                os.mkdir(args.output_model)

                if args.algorithm == "RTN":
                    dataloader = KVDataloader(os.path.join(args.model_path, model), pad_max=args.pad_max, batch_size=1)
                    op_type_dict={".*": {"weight": { "bits": 8, "group_size": args.group_size, "scheme": args.scheme, "algorithm": "RTN"}}}
                    config = PostTrainingQuantConfig(
                ...
                if args.algorithm == "AWQ":
                    dataloader = KVDataloader(os.path.join(args.model_path, model), pad_max=args.pad_max, batch_size=1)
                    op_type_dict={".*": {"weight": { "bits": 8, "group_size": args.group_size, "scheme": args.scheme, "algorithm": "AWQ"}}}
                    config = PostTrainingQuantConfig(
                        calibration_sampling_size=[8],
                        approach="weight_only",
                ...
                if args.algorithm == "GPTQ":
                    ...
                    op_type_dict={".*": {"weight": { "bits": 8, "group_size": args.group_size, "scheme": args.scheme, "algorithm": "GPTQ"}}}
                    config = PostTrainingQuantConfig(
                        approach="weight_only",
                        calibration_sampling_size=[8],
                        op_type_dict=op_type_dict)
